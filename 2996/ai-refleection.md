...
| OJ submission ID, if submitted | 542928 |
| OJ status | Pass|
...
|---|---|---|
| I read the relevant workflow before using AI. | Yes | STUDENT_WORKFLOW_CHATGPT_CODEX.md |
| I used `instructions/COURSE_AI_INSTRUCTIONS.md`, `instructions/AGENTS.md`, or manually followed the course AI instructions if the tool did not support custom instructions. | Yes | I used AGENTS.md |
| I wrote my own problem understanding before asking AI for help. | Yes | I already to understand and remember the guidance from ai |
| I wrote my own first plan before asking AI for help. | Yes | I try to frigue out what i  forgot but not untill i ask gemini|
| I used AI as a coach, reviewer, debugger, or test-case helper, not as a full-answer generator. | Yes | I used AI as a coach in this problrm |
...
```
ให้เไช่วยว่า คำสั่งไหนที่จะทำให้ ข้อความ รีเวิส
...
```
ทำความเข้าใจ ทบทวนความรู้ ที่เอไอแนะนำ และลองเทสเคสที่คิดเองอีกสามเทสเคสด้วยตัวเอง
...
```
ในการใช้เอไอช่วยทำโจทย์ในครั้งนี้ช่วยให้ได้ทบทวนความรู้ที่เรียนมาตอนพรีโปรว่าต้องใช้คำสั่งแบบนี้ จะได้ผลลัพะ์ออกมาแบบนี้ (reverse text)
